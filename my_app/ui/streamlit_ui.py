...
class StreamlitUI:
    ...
    def display_results(self, results, output_type):
        ...
        if output_type == "JSON":
            ...
            nodes, edges = self.process_query_results(results)
    ...
    def process_query_results(self, results):
        """Transforms query results into nodes and edges for visualization."""
        nodes = []
        edges = []  # Placeholder for edges, in case you add logic for edges later

        for record in results:
            # Assuming your query returns nodes as 'n', and record['n'] correctly fetches the node
            neo4j_node = record['n']

            # Extracting node ID, labels, and properties directly from the neo4j_node object
            node_id = str(neo4j_node.id)  # Node ID
            node_labels = ', '.join(neo4j_node.labels)  # Convert labels set to string
            node_properties = dict(neo4j_node)  # Convert node properties to a dictionary

            # Use a property for the title, or the labels if no suitable property found
            title = node_properties.get('name', node_properties.get('doc_id', node_labels))

            # Create a Node object for streamlit-agraph
            nodes.append(Node(id=node_id, label=title, title=title, size=400))
            # Note: Adjust 'label' and 'title' assignments as needed for your visualization

        # Assuming edges would be processed here similarly if included in your results
